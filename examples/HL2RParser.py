...
mymodel = {
          'HPtrNetPSTGate': HPtrNetPSTGate,
          'HPtrNetPSTSGate': HPtrNetPSTSGate,
          'HPtrNetPSGate': HPtrNetPSGate,
          }
...
def main():
    ...
    args_parser.add_argument('--grandPar', action='store_true', default=False, help='use grand parent.')
    args_parser.add_argument('--sibling', action='store_true', default=False, help='use sibling.')
    args_parser.add_argument('--proj', action='store_true', help='restricted to project trees.')
    ...
    args_parser.add_argument('--mymodel', help='mymodel implemented in parsing2, example EfficientPtrNet', required=True)
    ...
    data_train = conllx_stacked_data.read_stacked_data_to_tensor(train_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, prior_order=prior_order, device=device)
    num_data = sum(data_train[1])

    data_dev = conllx_stacked_data.read_stacked_data_to_tensor(dev_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, prior_order=prior_order, device=device)
    ...
    window = 3
    network = MyModel(word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, window,
                          ...
                          skipConnect=skipConnect, grandPar=grandPar, sibling=sibling, proj=proj, sgate=sgate)
    def save_args():
