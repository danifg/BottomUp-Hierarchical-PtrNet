...
class HPtrNetPSTGate(StackPtrNet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dep_hn_dense = nn.Linear(self.hidden_size, self.hidden_size)
        self.dep_hn_dense_gate = nn.Linear(self.hidden_size, self.hidden_size)

        self.cdep_hn_dense = nn.Linear(self.hidden_size, self.hidden_size)
        self.cdep_hn_dense_gate = nn.Linear(self.hidden_size, self.hidden_size)
        
        print('Setting Gate for Structural Decoding ...')
    ...
    def _get_decoder_output(self, output_enc, heads, heads_stack, siblings, hx, mask_d=None, length_d=None):
        ...
        if debug: print 'stack', heads_stack
        ...
        for step in range(length_decoder):
            ...
            hn_dep = [torch.zeros(hn[0].shape).type_as(hn[0]), torch.zeros(hn[1].shape).type_as(hn[1])]
            hn_cdep = [torch.zeros(hn[0].shape).type_as(hn[0]), torch.zeros(hn[1].shape).type_as(hn[1])]
            
            """

            for b in range(batch):
                curr_head = int(head[b])
                print 'curr_node', curr_head
                if (curr_head in parent_hn_dict) and (b in parent_hn_dict[curr_head]):
                    print 'Ya tiene un head'
                    for i in range(len(hn)):
            ...
            for b in range(batch):
                curr_node = int(head[b])
                #curr_head =int(heads[b, curr_node])
                if debug: print '________Curr_node', curr_node
                #Si el curr_node tiene un dep almacenado en el dict, lo cogemos
                if (curr_node in dep_hn_dict) and (b in dep_hn_dict[curr_node]):
                    if debug: print curr_node, 'SI tiene un dep'
                    for i in range(len(hn)):
                        hn_dep[i][:,b] = dep_hn_dict[curr_node][b][i].clone()

                if (curr_node in cdep_hn_dict) and (b in cdep_hn_dict[curr_node]):
                    for i in range(len(hn)):
                        hn_cdep[i][:,b] = cdep_hn_dict[curr_node][b][i].clone()
                        
            hn = list(hn)
            if self.sgate:
                for i in range(len(hn)):
                    tmp_hn_pre = self.pre_hn_dense(hn_pre[i])
                    tmp_hn_dep = self.dep_hn_dense(hn_dep[i].clone())
                    tmp_hn_cdep = self.cdep_hn_dense(hn_cdep[i].clone())
                    tmp_hn_dep_gate = self.dep_hn_dense_gate(hn_dep[i].clone() * hn_pre[i])
                    tmp_hn_cdep_gate = self.cdep_hn_dense_gate(hn_cdep[i].clone() * hn_pre[i]) 
                    gate = F.sigmoid(tmp_hn_dep_gate + tmp_hn_cdep_gate + self.bias_gate)
                    hn[i] = self.dropout_hn(F.tanh(tmp_hn_pre * gate + tmp_hn_dep * gate + tmp_hn_cdep * gate))
                ...
                for i in range(len(hn)):
                    ...
                    tmp_hn_dep_gate = self.dep_hn_dense_gate(hn_dep[i].clone())
                    tmp_hn_cdep_gate = self.cdep_hn_dense_gate(hn_cdep[i].clone()) 
                    ...
                    gate = F.sigmoid(tmp_hn_pre_gate + tmp_hn_dep_gate + tmp_hn_cdep_gate + self.bias_gate)
                    hn[i] = self.dropout_hn(F.tanh(tmp_hn_pre * gate + tmp_hn_dep * gate + tmp_hn_cdep * gate))
            ...
            for b in range(batch):
                ...
                for i in range(len(hn)):#usa el hn que se acaba de generar como siguiente decoder hidden state
            ...
            for b in range(batch):
                ...
                if curr_head not in dep_hn_dict:
                    dep_hn_dict[curr_head] = {}
                if curr_node < curr_head:
                    if b not in dep_hn_dict[curr_head]:
                        
                        dep_hn_dict[curr_head][b] = []
                        for i in range(len(hn)):
                            
                            dep_hn_dict[curr_head][b].append(hn[i][:,b].clone())

                if curr_head not in cdep_hn_dict:
                    cdep_hn_dict[curr_head] = {}
                if curr_node < curr_head:
                    cdep_hn_dict[curr_head][b] = []
                    for i in range(len(hn)):
                        cdep_hn_dict[curr_head][b].append(hn[i][:,b].clone()) 
    ...
    def _decode_per_sentence(self, output_enc, arc_c, type_c, hx, length, beam, ordered, leading_symbolic):
        ...
        def is_nonproj(A, head_node, node):
            ...
            for head, index in A:
                ...
                if int(index) > int(head):
                    left = int(head)
                    right = int(index)
                if (left<left_node and left_node<right and right<right_node) or (left_node<left and left<right_node and right_node<right):
                    return True
            return False
        ...
        children = torch.zeros(beam, int(length - 1)).type_as(output_enc).long()
        ...
        dep_hn_dict = [{} for i in range(beam)]
        cdep_hn_dict = [{} for i in range(beam)]
        """
        ...
        for t in range(num_step):
            ...
            hx_dep = [torch.zeros(hx[0].shape).type_as(hx[0]), torch.zeros(hx[1].shape).type_as(hx[1])]
            hx_cdep = [torch.zeros(hx[0].shape).type_as(hx[0]), torch.zeros(hx[1].shape).type_as(hx[1])]
            
            # update parent hidden states

            for n in range(num_hyp):
                head = int(heads[n])
                if debug: print 'Se busca dep para curr_node', head
                ...
                if head in dep_hn_dict[n]:
                    if debug: print 'SI tiene dep'
                    for i in range(len(hx)):
                        hx_dep[i][:,n,:] = dep_hn_dict[n][head][i].clone()
                        
                if head in cdep_hn_dict[n]:
                    for i in range(len(hx)):
                        hx_cdep[i][:,n,:] = cdep_hn_dict[n][head][i].clone()
            ...
            if self.sgate:
                for i in range(len(hx)):
                    ...
                    tmp_hn_dep = self.dep_hn_dense(hx_dep[i].clone())
                    tmp_hn_cdep = self.cdep_hn_dense(hx_cdep[i].clone())
                    tmp_hn_dep_gate = self.dep_hn_dense_gate(hx_dep[i].clone() * hx_pre[i])
                    tmp_hn_cdep_gate = self.cdep_hn_dense_gate(hx_cdep[i].clone() * hx_pre[i])
                    gate = F.sigmoid(tmp_hn_dep_gate + tmp_hn_cdep_gate + self.bias_gate)
                    hx[i] = self.dropout_hn(F.tanh(tmp_hn_pre * gate + tmp_hn_dep * gate + tmp_hn_cdep * gate))
            else:
                for i in range(len(hx)):
                    tmp_hn_pre = self.pre_hn_dense(hx_pre[i])
                    ...
                    tmp_hn_dep_gate = self.dep_hn_dense_gate(hx_dep[i].clone())
                    tmp_hn_cdep_gate = self.cdep_hn_dense_gate(hx_cdep[i].clone())
                    #gate = F.sigmoid(tmp_hn_pre_gate + tmp_hn_parent_gate + tmp_hn_sib_gate + self.bias_gate)
                    gate = F.sigmoid(tmp_hn_pre_gate + tmp_hn_dep_gate + tmp_hn_cdep_gate + self.bias_gate)
            ...
            new_dep_hn_dict = [{} for i in range(beam)]
            new_cdep_hn_dict = [{} for i in range(beam)] 
            ...
            for id in range(num_hyp * length):
                ...
                if self.proj and is_nonproj(arcs[base_id], int(child_id), int(head)):
                    continue
                
                if hasCycles(arcs[base_id], int(child_id), int(head)):
                    continue

                if debug: print 'current node', int(head)
                if debug: print 'head asignado', int(child_id)
                

                new_arcs[cc] = set(arcs[base_id])
                new_arcs[cc].add((int(child_id),int(head)))
                ...
                if debug: print 'heads ya asignados', new_children
                ...
                """
                int_curr_node = int(head)
                int_assigned_head = int(child_id)
                new_dep_hn_dict[cc] = copy.copy(dep_hn_dict[base_id])
                new_cdep_hn_dict[cc] = copy.copy(cdep_hn_dict[base_id])
                if int_curr_node < int_assigned_head:#left arc
                    if int_assigned_head not in new_dep_hn_dict[cc]:
                        new_dep_hn_dict[cc][int_assigned_head] = []
                        for i in range(len(hx)):
                            new_dep_hn_dict[cc][int_assigned_head].append(hx[i][:,base_id,:].clone()) 
                        if debug: print 'Se asigna ', int(head), 'como dep de', int_assigned_head

                    new_cdep_hn_dict[cc][int_assigned_head] = []
                    for i in range(len(hx)):
                        new_cdep_hn_dict[cc][int_assigned_head].append(hx[i][:,base_id,:].clone())
            ...
            if num_hyp == 0:
                ...
                index = base_index.new(1).fill_(ids[0]) #Lo cogemos de SeqL2R aunque no se si hace falta
            else:
            ...
            dep_hn_dict = new_dep_hn_dict
            cdep_hn_dict = new_cdep_hn_dict
        ...
        if debug: print 'heads', heads, 'types', types
        if debug: exit(0)
